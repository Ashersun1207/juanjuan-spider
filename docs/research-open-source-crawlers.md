...
```
juanjuan-spider (scrape.py 统一入口)
    │
    ├── L0 自建层（免费，我们控制）
    │     ├── Crawl4AI 引擎 → 通用网页（已集成）
    │     ├── 中文社交平台适配 → 参考 MediaCrawler 思路自写
    │     └── 登录态管理 / 站点适配器
    │
    ├── L1 免费服务层（零成本 fallback）
    │     ├── Jina Reader (r.jina.ai) → 简单页面，10M token 免费
    │     ├── web_fetch (OpenClaw 内置) → 轻量抓取
    │     └── Tavily extract → 受保护页面（1000次/月免费）
    │
    └── L2 付费服务层（硬骨头专用）
          ├── 候选服务（待选型）
          └── Cloudflare 重防护 / 大规模抓取 / 高可用场景
```
...
| 服务 | 起步价 | 反爬能力 | 特点 |
|---|---|---|---|
| **Scrape.do** | $29/250K 次 | ✅ 好 | 性价比最高（$0.80/千次），有免费 trial |
| **ScrapingBee** | $49/250K 次 | ⚠️ 一般 | 老牌，文档好，反爬 32% 成功率 |
| **ZenRows** | $69/250K 次 | ✅ 强 | 反爬最强，但贵 |
| **Scrapfly** | $30/200K 次 | ✅ 最强(98%) | 反爬成功率最高，按域名计费复杂 |
| **Browserless** | $0 自部署 / $200+托管 | ✅ 好 | 自部署免费，云端贵 |

**建议**：先不买。等真正遇到 L0 搞不定的场景，再按需选。优先试 Scrape.do（性价比）或 Scrapfly（成功率）。
...
URL 进来
  ↓
是中文社交平台？→ L0 中文适配器（小红书/微博/知乎...）
  ↓ 否
L0 Crawl4AI 抓取
  ↓ 失败？
L1 Jina Reader / Tavily fallback
  ↓ 还失败？
L2 付费 API（如果已配置）
  ↓ 未配置？
返回错误 + 建议
...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **P0** | Crawl4AI 集成（L0 通用引擎） | ✅ 已做 |
| **P0** | Jina Reader fallback（L1） | 待集成，零成本 |
| **P1** | 中文社交平台适配（L0） | 参考 MediaCrawler 思路自写 |
| **P1** | 登录态管理 | 参考 MediaCrawler Playwright 方案 |
| **P2** | 路由逻辑 + 自动 fallback | L0→L1→L2 |
| **P2** | L2 付费服务接入 | 按需选型 |
| **P3** | 架构优化（队列/并发） | 借鉴 Crawlee 设计 |
